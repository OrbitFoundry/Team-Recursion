...
type AdminModeratedResource = Resource & {
  student: {
    name: string;
    email: string;
    _id: string;
...
function AdminResourcesContent() {
  const [resources, setResources] = useState<AdminModeratedResource[]>([]);
  ...
  const fetchResources = useCallback(async () => {
    try {
      ...
      setResources(data as AdminModeratedResource[]);
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      await adminApi.deleteResource(deleteId);
              ...
              resources.map((r) => (
                ...
                >
                  <div>
                    ...
                    <h3 className="font-bold text-gray-900 dark:text-white text-base leading-snug mb-1 font-sans">
                      {r.title}
                    </h3>
                    <div className="text-[10px] font-mono text-gray-500 dark:text-gray-400">
                      Uploaded by: <span className="font-bold">{r.student?.name || 'Unknown Student'}</span> ({r.student?.email || 'N/A'})
                    </div>
                  </div>
