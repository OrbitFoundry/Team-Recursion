...
const findBackendDirectory = () => {
    const cwd = process.cwd();
    // Check if we're already in the backend directory
    const currentPackageJson = path_1.default.resolve(cwd, 'package.json');
    if ((0, node_fs_1.existsSync)(currentPackageJson)) {
        try {
            const packageContent = JSON.parse((0, node_fs_1.readFileSync)(currentPackageJson, 'utf-8'));
    ...
    const parentBackendPackageJson = path_1.default.resolve(cwd, '..', 'backend', 'package.json');
    if ((0, node_fs_1.existsSync)(parentBackendPackageJson)) {
        try {
            const packageContent = JSON.parse((0, node_fs_1.readFileSync)(parentBackendPackageJson, 'utf-8'));
            if (packageContent.name === 'backend') {
                return path_1.default.resolve(cwd, '..', 'backend');
            }
        }
        catch {
...
const envFilePath = path_1.default.resolve(backendDir, '.env');
let envLoaded = false;
let loadError = null;
let loadedEnvPath = null;
const isDevelopment = process.env.NODE_ENV === 'development';
// Helper function for early logging (before logger is available)
const earlyLog = (message, isError = false) => {
...
if (missingVars.length > 0) {
    ...
    const envExamplePath = path_1.default.resolve(backendDir, 'env.example');
    const envPath = path_1.default.resolve(backendDir, '.env');
